...
export function parseFlags(argv) {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (equals >= 0) flags[raw.slice(0, equals)] = raw.slice(equals + 1);
...
export function matchesAny(filePath, patterns = []) { return patterns.some((pattern) => matchesGlob(filePath, pattern)); }
export function walkCodeFiles(root, options = {}) {
  ...
  while (stack.length && files.length < maxFiles) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) { stack.push(relative); continue; }
