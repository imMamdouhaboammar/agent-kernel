...
- **Multi-Scoped Persistent Memory Engine (`ak memory`)**: Multi-scoped (`user`, `project`, `agent`, `episode`) persistent memory layer with atomic `FactNode` schema, hybrid (semantic + keyword) search, and automatic conflict reconciliation state machine (`ak memory extract/search/reconcile`).
