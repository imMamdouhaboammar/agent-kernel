...
//   4. CLI command surfaces are wired
//   5. MCP tool names use the agent_kernel_ prefix and are listed
//   6. Secret patterns in src/cli.mjs are still present
//   7. Deny commands in src/cli.mjs are still present
//   8. engines.node is set
//   9. package.json `name` and README install command agree
//  10. README badge URLs reference the same package name
//  11. CHANGELOG latest release header matches package.json version
//  12. package.json `files` whitelist is intentional
//  13. `develpment/` legacy typo points to `development/`
//  14. No stale `@mamdouh/agent-kernel` references in active docs
//  15. No hardcoded secrets in public repository files
//
...
else if (cliVersionMatch[1] !== pkg.version) err(`VERSION (${cliVersionMatch[1]}) != package.json (${pkg.version})`);
...
// 6. Secret patterns still present in runtime policy
...
const denyCommands = ['dangerous-rm', 'curl-pipe-shell', 'chmod-777', 'force-push-main', 'delete-git'];
let missingDenies = 0;
...
if (existsSync(readmePath)) {
  ...
  if (!m) err('README.md has no `npm install -g <package>` example');
  else if (m[1] !== pkg.name) err(`README install command uses ${m[1]} but package.json name is ${pkg.name}`);
  else ok(`README install command matches package.json name (${pkg.name})`);
}
...
if (existsSync(readmePath)) {
  ...
  if (total === 0) err('README has no npm/v, npm/dw, or bundlephobia/min badges');
  else if (mismatched === 0) ok(`${total} npm/bundlephobia badges reference ${pkg.name}`);
}
...
if (existsSync(changelogPath)) {
  ...
  if (!m) err('CHANGELOG.md has no `## [version]` header');
  else if (m[1] !== pkg.version) err(`CHANGELOG.md latest header is ${m[1]} but package.json is ${pkg.version}`);
  else ok(`CHANGELOG latest entry matches package.json (${pkg.version})`);
}
...
if (Array.isArray(pkg.files)) {
  ...
  if (missing.length > 0) err(`package.json files whitelist missing required entries: ${missing.join(', ')}`);
  else ok('package.json files whitelist has all required entries');
} else {
  err('package.json has no `files` array');
}

// 13. develpment/ compatibility pointer
...
if (existsSync(legacyDir) && !existsSync(canonicalDir)) {
  ...
  if (existsSync(legacyReadme)) {
    ...
    if (!text.includes('development/')) err('develpment/README.md does not point to development/ as the canonical path');
    else ok('develpment/ compatibility pointer references development/');
  } else {
    err('develpment/ exists but has no README.md compatibility pointer');
  }
} else if (!existsSync(legacyDir) && !existsSync(canonicalDir)) {
  ok('no develpment/ or development/ folder present');
}
...
function walkFiles(dir, predicate, results = []) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (rel.startsWith('.agent-kernel-backups') || rel.startsWith('agent-kernel-export')) continue;
...
// 14. No stale npm scope references in active docs
...
// 15. No hardcoded secrets in repository files
const scanExtensions = /\.(mjs|js|json|yml|yaml|toml|md|sh)$/;
const secretRegexes = [
...
let secretLeaks = 0;
for (const f of filesToSecretScan) {
  const rel = relative(root, f).replace(/\\/g, '/');
  ...
  for (const pattern of secretRegexes) {
    ...
    if (match) {
      err(`possible hardcoded secret in ${rel}: ${match[0].slice(0, 12)}...`);
      secretLeaks++;
      break;
    }
  }
}
if (secretLeaks === 0) ok(`no hardcoded secrets in ${filesToSecretScan.length} scanned repository files`);
